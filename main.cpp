...
int main()
{
	...
	while (true)
	{
		...
		// get eye position ( origin + viewoffset )
		...
		// to customize fov, change best fov num
		...
		for (auto i = 1; i <= 32; ++i)
		{
			...
			// if we have best angle and all conditions are met, shoot aim gun kill
			if (!bestAngle.IsZero())
				memory.Write<Vector3>(clientState + offset::dwClientState_ViewAngles, viewAngles + bestAngle / 3.f);

			// 3 is the smoothing of the aimbot, the higher the number you divide by, the smoother the aimbot is.
		}
	}
